...
//여기서 import한 cn은 function이락 생각하고 사용하면 된다.
import cn from "classnames";
import binder from "classnames/bind";
//classnames 의 binder을 이용하여 모듈와된 css를 바인딩해서 그 참조값을 cx에 대입하기
    ...
    render(){
        return(
            <div className="container">
                <h3>모듈화된 css사용의 불편함 해결하기</h3>
                <div className = "box bg-yellow">box</div>
                <div className = {"box bg-yellow"}>box2</div>
                {/* 적용할 클래스 문자열로 나열 */}
                <div className={cn('box', 'bg-yellow')}>box3</div>
                {/* cn({}) object를 전달해서 true, false를 활용해서 class를 거부할지 말지를 결정 */}
                <div className = {cn({box:true, "bg-yellow":true})}> div4 </div>  
                {/* object와 문자열 조합하여도 사용 가능 */}      
                <div className = {cn("box", {"bg-yellow":true})}> div5 </div>    
